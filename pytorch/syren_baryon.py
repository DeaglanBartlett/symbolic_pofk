...
def syren_baryon_Astrid(k, z, Omega_m, sigma_8, A_SN1, A_SN2, A_AGN1, A_AGN2):
    ...
    alpha_10 = 0.029
    alpha_11 = 0.063
    log_s = alpha_1*k*(alpha_2*k)**alpha_3*(alpha_4 - z)/((alpha_5*z - 1/alpha_6**(2*z))*(alpha_7 + alpha_8*k + k**2)) + (alpha_9*k - 1 + torch.exp(-alpha_10*k))*torch.exp(-alpha_11*z)
    s = torch.exp(log_s)
...
def syren_baryon_IllustrisTNG(k, z, Omega_m, sigma_8, A_SN1, A_SN2, A_AGN1, A_AGN2):
    ...
    alpha_9 = 0.021
    alpha_10 = 0.797
    log_s = -alpha_1*alpha_2**(-alpha_3*k - alpha_4*z)*k*(alpha_5*(-alpha_6*k + alpha_7**z) + 1) + alpha_8*torch.exp(-alpha_9*torch.exp(alpha_10*z)/k)
    s = torch.exp(log_s)
...
def syren_baryon_SIMBA(k, z, Omega_m, sigma_8, A_SN1, A_SN2, A_AGN1, A_AGN2):
    ...
    alpha_10 = 114.529
    alpha_11 = 1.21
    log_s = -alpha_1*k*(alpha_2 + k)/((alpha_5*z + (alpha_3*k)**(alpha_4*z))*(alpha_7*z + alpha_8 + k**alpha_6)) + alpha_9*torch.exp(-alpha_10*torch.exp(-alpha_11*z)/k - 2*z)
    s = torch.exp(log_s)
...
def syren_baryon_Swift_EAGLE(k, z, Omega_m, sigma_8, A_SN1, A_SN2, A_AGN1, A_AGN2):
    ...
    alpha_16 = 130.0
    alpha_17 = 0.565
    log_s = alpha_1**(alpha_2/k + alpha_3*k + alpha_4*z)*(alpha_14 - alpha_5*k + alpha_6*z + (alpha_10*k + alpha_11)/(alpha_12*z + alpha_13 + k) + (alpha_7*k*z + k**2)/(alpha_8*k + alpha_9)) + alpha_15*torch.exp(-alpha_16*torch.exp(-z)/k - torch.exp(alpha_17*z))
    s = torch.exp(log_s)
...
def epsilon_Astrid(k, z):
    ...
    alpha_3 = 1.3
    alpha_4 = 0.25285
    epsilon = alpha_1*k/(alpha_2*k*torch.exp(-alpha_3*z) + torch.exp(alpha_4*z))
