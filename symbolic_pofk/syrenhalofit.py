...
def run_halofit(k, sigma8, Om, Ob, h, ns, a, emulator='fiducial', extrapolate=True, which_params='Bartlett', add_correction=True):
    ...
    # Get linear P(k)
    plin = linear.plin_emulated(k, sigma8, Om, Ob, h, ns, a=a, emulator=emulator, extrapolate=extrapolate)
    ...
    alpha = np.abs(pars[22] + pars[23] * neff + pars[24] * neff ** 2 + pars[25] * C)
    ...
    pq = plin * (1 + deltaL2) ** beta / (1 + alpha * deltaL2) * np.exp(- y/4 - y**2/8)
