...
def pnl_new_emulated(k, theta_batch):
    """
    ...
    P_lin = torch.log10(plin_new_emulated(k, theta_batch).T)
    ...
    denominator1_part1 = (g[4] * k_batch**(-g[5]) - g[6] *
                          P_lin)**(g[7] * P_lin + g[8] * wa + g[9] * w0 - g[10])
    denominator1_part2 = (g[11] * k_batch**g[12] + g[13]
...
def pnl_bias(k_batch):
    ...
    Returns:
        :offset (torch.Tensor): The offset of the emulated non-linear log_10 P(k)
    """

    h = torch.tensor([0.5787,   2.3485,  27.3829,  16.4236,  97.3766,  90.9764,
                      11.2046,  2447.2, 11376.93], device=device)

    term1 = ((h[1] * k_batch) - torch.cos(h[3] *
             torch.cos(h[2] * k_batch)))*torch.cos(h[4] * k_batch)
