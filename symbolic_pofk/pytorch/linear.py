...
def pk_EisensteinHu_zb(k, theta_batch, integral_norm=True):
    ...
    if integral_norm:
        ...
        def get_pk(kk, Anorm):
            ...
            L0 = torch.log(2.0 * torch.exp(torch.tensor(1.0, device=device)) + 1.8 * q)
            tk_eh = L0 / (L0 + C0 * q**2)
        ...
        b0 = torch.log(torch.tensor(1e-7, dtype=torch.float32, device=device))  # ln(k_min)
        b1 = torch.log(torch.tensor(1e5, dtype=torch.float32, device=device))  # ln(k_max)
...
def sigma8_to_As(theta_batch, old_equation=False):
    ...
    if old_equation:
        a = torch.tensor([0.161320734729, 0.343134609906, - 7.859274, 18.200232, 3.666163, 0.003359], device=device)
